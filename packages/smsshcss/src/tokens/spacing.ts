...
// 設定からスペーシングのトークンを解決する関数
export function resolveSpacing(config?: SmsshcssConfig) {
  return resolveTokens('spacing', baseSpacing, config?.theme);
}

// デフォルトのスペーシングのトークン
export const spacing = { ...baseSpacing } as const;